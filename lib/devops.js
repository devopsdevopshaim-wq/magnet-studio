...
//  • n8n: לוח בקרה ל-n8n המקומי (workflows, הרצות, הפעלה/כיבוי, הרצה ידנית)
...
const DEFAULT_CFG = {
  n8n: {
    base: "http://localhost:5680",
    container: "magnet-studio-n8n",
    apiKey: "" // נוצר ב-n8n: Settings → n8n API → Create API key
  },
  roots: [ALLOWED_ROOT],
...
async function n8nStatus() {
  ...
  const up = await reachable(cfg.base + "/healthz").catch(() => false) || await reachable(cfg.base);
  ...
  return { base: cfg.base, container: cfg.container, up: !!up, hasKey: !!cfg.apiKey, apiOk, apiError };
}
...
// הרצה ידנית — דרך ה-CLI בקונטיינר (לא דורש שה-workflow יהיה webhook)
async function n8nRun(id) {
  const cfg = readConfig().n8n;
  if (!(await dockerAvailable())) return { ok: false, error: "Docker לא פעיל" };
  const r = await run("docker", ["exec", cfg.container, "n8n", "execute", "--id", String(id)], { timeout: 120000 });
  const tail = (r.stdout + "\n" + r.stderr).trim().split("\n").slice(-12).join("\n");
  return { ok: r.ok, output: tail || (r.ok ? "בוצע" : "נכשל") };
}
...
async function infra() {
  ...
  const checks = [
    ...
    { name: "n8n — JARVIS", url: readConfig().n8n.base + "/healthz" },
