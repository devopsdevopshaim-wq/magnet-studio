// פרוקסי צד-שרת לצ'אט AI (workflow "JARVIS" ב-n8n Cloud — הרשת של הבעלים, לא Docker מקומי).
// מטרתו לפתור CORS (הדפדפן מדבר מול same-origin /api/jarvis/ask במקום ישירות מול n8n)
// ולאפשר גם לנרטיב היומי להשתמש באותו סוכן. שום מפתח לא נדרש - ה-Chat Trigger של n8n
// פתוח לפי ה-webhook id. תמיד מול n8n Cloud — אין יעד מקומי (הוסר בכוונה: הצ'אט צריך
// לעבוד מכל מכשיר/רשת, לא רק כשה-Docker המקומי דלוק).
//
// n8n מקומי (Docker, 5680) עדיין קיים באפליקציה לצרכים אחרים (ראו server.js /api/n8n/local-*) —
// הוא לא קשור לצ'אט AI.
...
function readConfig() {
  ...
  return {
    target: "cloud", // נשמר לתאימות לאחור עם קוד שקורא cfg.target (תמיד cloud כעת)
...
async function ask(chatInput, sessionId, { timeout } = {}) {
  ...
  let hint;
  if (res.status === 404) {
    hint = "ה-workflow של הצ'אט אינו Active ב-n8n Cloud, או שמזהה ה-Webhook שגוי.";
