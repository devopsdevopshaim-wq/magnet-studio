...
const CONFIG_FILE = path.join(PERSIST_DIR, "jarvis-config.json");

// בענן (Render וכו') אין Docker/n8n מקומי לפנות אליו — DISABLE_DOCKER_ORCHESTRATION="1" הוא הדגל
// הקיים שמסמן "רצים מכולה/ענן, בלי תזמור Docker". ברירת המחדל שם היא cloud, לא local.
const DEFAULTS = {
  target: process.env.DISABLE_DOCKER_ORCHESTRATION === "1" ? "cloud" : "local", // local | cloud
...
function readConfig() {
  ...
  // בענן (Docker orchestration כבוי) אין שום דרך להגיע ל-n8n מקומי — גם אם המשתמש שמר בעבר
  // target:"local" (למשל דרך המחשב), מתעלמים מזה כאן כדי לא להיתקע בשקט על webhook שלא ניתן להשגה.
  const inCloud = process.env.DISABLE_DOCKER_ORCHESTRATION === "1";
  return {
    target: inCloud ? "cloud" : (saved.target === "cloud" || saved.target === "local" ? saved.target : DEFAULTS.target),
...
function writeConfig(partial) {
  ...
  const inCloud = process.env.DISABLE_DOCKER_ORCHESTRATION === "1";
  next.target = inCloud ? "cloud" : (next.target === "cloud" || next.target === "local" ? next.target : DEFAULTS.target);
