...
// מפעל (factory): require("./aiaStudio")(baseDir) מחזיר גלריית פרויקטים פרטית לתיקייה הזו —
// כך שלכל חשבון (בעלים או משתמש רשום) יש גלריית AIA נפרדת משלו.
...
function createStore(baseDir) {
  const AIA_DIR = path.join(baseDir || PERSIST_DIR, "aia");
  ...
  return {
    ...
    stageImage, clearStage, attachRender, purgeStaging,
    AIA_DIR
  };
}

createStore.MAX_IMAGES = MAX_IMAGES;
