...
// מאוחסן ב-data/astro-config.json. כל החישוב האסטרולוגי/אסטרונומי מקומי - שום דבר לא נשלח החוצה.

const fs = require("fs");
const path = require("path");

const DATA_DIR = path.join(__dirname, "..", "data");
const FILE = path.join(DATA_DIR, "astro-config.json");
 ...
 */
function writeAstroConfig(input) {
  const b = input || {};
...
module.exports = { readAstroConfig, writeAstroConfig, israelTzOffsetMinutes, CITIES, FILE };
