...
//
// אחסון: PERSIST_DIR (ראו lib/paths.js) — עם דיסק קבוע מחובר (render.yaml) שורד פריסה מחדש בענן.

const fs = require("fs");
const path = require("path");
const { PERSIST_DIR } = require("./paths");

const CONFIG = path.join(PERSIST_DIR, "integrations-config.json");
...
const REGISTRY = [
  ...
  },
  {
    ...
    fields: [
      simpleEnvField("OPENWEBUI_BASE_URL", "כתובת נגישה מהאינטרנט (לא localhost אם זה שרת בענן)", { type: "text", placeholder: "http://127.0.0.1:3000" }),
      simpleEnvField("OPENWEBUI_API_KEY", "מפתח API")
    ],
    status: async () => {
      const reachable = await require("./aiPanel").isOpenWebUIReachable().catch(() => false);
      return {
        configured: envConfigured("OPENWEBUI_API_KEY") && reachable,
        detail: reachable ? "מגיב" : envConfigured("OPENWEBUI_BASE_URL") ? "לא מגיב בכתובת שהוגדרה" : "לא זמין במחשב הזה"
      };
    },
    save: (v) => { setEnv("OPENWEBUI_BASE_URL", v.OPENWEBUI_BASE_URL); setEnv("OPENWEBUI_API_KEY", v.OPENWEBUI_API_KEY); }
  },
  {
    id: "ollama", label: "Ollama", category: "ai", vendor: "Ollama — חינם, בלי מפתח",
    docsUrl: "https://ollama.com",
    fields: [
      simpleEnvField("OLLAMA_BASE_URL", "כתובת (ריק = המחשב הזה בלבד)", { type: "text", placeholder: "http://127.0.0.1:11434" })
    ],
    status: async () => {
      const models = await require("./aiPanel").listOllamaModels().catch(() => []);
      const custom = envConfigured("OLLAMA_BASE_URL");
      return {
        configured: models.length > 0,
        detail: models.length ? `${models.length} מודלים` : (custom ? "לא מגיב בכתובת שהוגדרה" : "לא זמין במחשב הזה")
      };
    },
    save: (v) => setEnv("OLLAMA_BASE_URL", v.OLLAMA_BASE_URL)
  },
  {
    id: "jarvis", label: "צ'אט AI (n8n Cloud)", category: "ai", vendor: "n8n Cloud",
    ...
    status: async () => {
      ...
      return { configured: true, detail: `n8n Cloud (${cfg.base})` };
    },
    save: (v) => require("./jarvisClient").writeConfig({ base: v.__jarvis_base || undefined }),
    reveal: () => { const cfg = require("./jarvisClient").readConfig(); return { __jarvis_base: cfg.base }; }
  },
  ...
  // ---------- מדיה ----------
  {
    ...
    reveal: () => require("./ttsClient").getConfig()
  },
  {
    ...
    reveal: () => ({ key: require("./aiaVideo").getKey("seedance") })
  },
  {
...
// הערכים המלאים (לא מוסתרים) — נחשף רק כשהמשתמש מבקש במפורש "הצג". לא מציג סיסמת-אתר בטקסט גלוי.
async function reveal(id) {
  const r = REGISTRY.find((x) => x.id === id);
  if (!r) throw new Error("אינטגרציה לא מוכרת");
  if (typeof r.reveal === "function") return r.reveal();
  const out = {};
  for (const f of r.fields || []) {
    if (f.type === "select" || f.key === "password") continue;
    out[f.key] = process.env[f.key] || "";
  }
  return out;
}

module.exports = { list, save, reveal, setEnv };
