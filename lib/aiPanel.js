...
// ברירת מחדל: המחשב המקומי. כדי שהשרת בענן (Render/הטלפון) יגיע לאולאמה שרץ בבית — צריך כתובת
// שנגישה מהאינטרנט (למשל טאנל כמו Tailscale Funnel / Cloudflare Tunnel / ngrok), לא IP מקומי —
// "127.0.0.1" מהקונטיינר של Render הוא הקונטיינר עצמו, לא המחשב הביתי. ראו OLLAMA_BASE_URL.
function ollamaBase() {
...
async function listOllamaModels() {
  try {
    const r = await httpJson(`${ollamaBase()}/api/tags`, { timeout: 4000 });
...
async function queryOllama(model, prompt) {
  const r = await httpJson(`${ollamaBase()}/api/generate`, {
    method: "POST",
...
const CLOUD_SOURCES = [
  {
    ...
    async ask(prompt, apiKey) {
      const r = await httpJson("https://api.anthropic.com/v1/messages", {
        ...
        body: { model: "claude-sonnet-5", max_tokens: 1024, messages: [{ role: "user", content: prompt }] }
      });
  ...
  },
  {
    ...
    async ask(prompt, apiKey) {
      const r = await httpJson(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.8-flash:generateContent?key=${apiKey}`, {
        method: "POST",
...
module.exports = { listSources, askAll, synthesize, listOllamaModels, queryOllama, isOpenWebUIReachable, CLOUD_SOURCES };
