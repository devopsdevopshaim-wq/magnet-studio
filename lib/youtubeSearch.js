...
async function searchYouTube(query, max = 12) {
  const q = String(query || "").trim();
  if (!q) return [];
  const url = `https://www.youtube.com/results?search_query=${encodeURIComponent(q)}&hl=he`;
  ...
  (function walk(node) {
    ...
    if (node.videoRenderer) {
      ...
      if (id && title && !seen.has(id)) {
        seen.add(id);
        out.push({ id, title: title.trim(), channel: channel.trim(), duration: dur });
      }
      return;
    }
...
module.exports = { searchYouTube, secToClock };
