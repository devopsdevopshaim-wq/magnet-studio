...
const { DATA_DIR, PERSIST_DIR } = require("./paths");
...
const BACKUP_FILES = [
  ...
  "jarvis-config.json", // יעד n8n (מקומי/ענן) + webhook — חי ב-PERSIST_DIR
...
// jarvis-config.json חי ב-PERSIST_DIR (ראו lib/paths.js); כל השאר ב-DATA_DIR הרגיל
function dirFor(name) { return name === "jarvis-config.json" ? PERSIST_DIR : DATA_DIR; }

function readJsonRaw(name) {
  try {
    const txt = fs.readFileSync(path.join(dirFor(name), name), "utf8");
  ...
  for (const [name, content] of Object.entries(bundle.files)) {
    ...
    try {
      ...
      if (fs.existsSync(dest)) {
        fs.copyFileSync(dest, path.join(dir, `${name}.pre-import-${stamp}.bak`));
      }
