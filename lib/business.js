...
// מפעל (factory): require("./business")(baseDir) מחזיר חנות נתונים פרטית לתיקייה הזו —
// כך שלכל חשבון (בעלים או משתמש רשום) יש עסק נפרד משלו. אין תלות ב-API חיצוני.
...
function createStore(baseDir) {
  const DIR = path.join(baseDir || PERSIST_DIR, "business");
  const FILES = {
