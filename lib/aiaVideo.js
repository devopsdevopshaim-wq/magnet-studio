...
// מפתחות ה-API של ספקי הווידאו נשארים תשתית משותפת (לא פרטיים למשתמש) —
// אבל קובצי הפלט עצמם נכתבים לתוך aiaDir שמגיע מהקורא (per-account, ראו server.js)
...
const PROVIDERS = {
  ...
  hunyuan: {
    id: "hunyuan",
    label: "HunyuanVideo",
    vendor: "Tencent · דרך fal.ai",
    site: "https://fal.ai/models/fal-ai/hunyuan-video",
    keyEnv: "FAL_KEY",                     // אותו מפתח fal.ai כמו Seedance — אם כבר מוגדר, זה כבר מחובר
    keyHint: "מפתח fal.ai (מתחיל ב-\"fal-\" או זוג key:secret) — fal.ai/dashboard/keys",
    defaultModel: "fal-ai/hunyuan-video",
    durations: [5],                        // המודל קובע אורך קבוע לפי מספר פריימים; אין שליטה חופשית
    aspects: ["16:9", "9:16", "1:1"]
  }
};
...
// מנוע גנרי לתור fal.ai (submit → poll status → הורדת הקובץ הסופי) — Seedance ו-HunyuanVideo
// חולקים בדיוק את אותה זרימת API, רק שם המודל וגוף הבקשה שונים.
async function runFalQueue({ aiaDir, job, project, key, label, model, body, durationSec }) {
  job.phase = `שולח ל-${label}`; job.pct = 8;
  ...
  if (!submit.ok) throw new Error(`${label}: ` + (sj.detail || sj.error || submit.status));
  ...
  job.phase = `${label} מייצר וידאו`; job.pct = 20;
  const started = Date.now();
  while (Date.now() - started < 8 * 60 * 1000) {
    await new Promise((r) => setTimeout(r, 4000));
    const st = await fetch(statusUrl, { headers: { "Authorization": "Key " + key } }).then((r) => r.json()).catch(() => ({}));
    if (st.status === "COMPLETED") break;
    if (st.status === "FAILED" || st.error) throw new Error(`${label} נכשל: ` + (st.error || "שגיאת מנוע"));
    job.pct = Math.min(75, job.pct + 3);
    if (st.queue_position != null) job.phase = `בתור ב-${label} (${st.queue_position})`;
  }
  ...
  if (!videoUrl) throw new Error(`${label} לא החזיר קובץ וידאו`);
  const dest = path.join(aiaDir, "renders", `${project.id}.mp4`);
  await downloadTo(videoUrl, dest);
  return { file: `/api/aia/render/${project.id}.mp4`, durationSec };
}

async function runSeedance(aiaDir, job, project, opts) {
  ...
  return runFalQueue({ aiaDir, job, project, key, label: "Seedance", model, body, durationSec: Number(body.duration) || 8 });
}

async function runHunyuan(aiaDir, job, project, opts) {
  const key = keyFor("hunyuan");
  const cfg = readConfig();
  const pcfg = (cfg.providers && cfg.providers.hunyuan) || {};
  const model = pcfg.model || PROVIDERS.hunyuan.defaultModel;
  const prompt = buildPrompt(project, opts);
  ...
  return runFalQueue({ aiaDir, job, project, key, label: "HunyuanVideo", model, body, durationSec: 5 });
}

async function runDeevid(aiaDir, job, project, opts) {
...
async function attachUpload(aiaDir, project, dataUrl) {
  ...
  const RENDER_DIR = path.join(aiaDir, "renders");
...
async function submit(aiaDir, project, opts = {}) {
  ...
  const runner = pid === "seedance" ? runSeedance : pid === "hunyuan" ? runHunyuan : runDeevid;
  runner(aiaDir, job, project, opts)
    .then((res) => {
