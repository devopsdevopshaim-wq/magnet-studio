...
// טוקן ביקור גס לצרכי אנליטיקס בלבד (לא אימות) — ערך גולמי של עוגיית ההתחברות, אם יש
function visitorToken(req) {
  return parseCookies(req.headers.cookie)[COOKIE] || null;
}

module.exports = { config, gate, checkPassword, setSession, clearSession, isAuthed, setPassword, visitorToken };
