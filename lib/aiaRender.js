...
// AIA_DIR המשותף משמש רק לתשתית לא-פרטית (מיטת מוזיקה מסונתזת, תיקיית עבודה זמנית) —
// נתוני משתמש בפועל (תמונות, רינדורים) נגזרים מ-aiaDir שמגיע מהקורא (per-account, ראו server.js)
const AIA_DIR = path.join(PERSIST_DIR, "aia");
const WORK_DIR = path.join(AIA_DIR, "_work");
const BEDS_DIR = path.join(AIA_DIR, "beds");

// נתיבי דג'אווו שונים בין הפצות: Debian/Ubuntu שם אותו תחת truetype/dejavu, Alpine (Dockerfile שלנו) תחת dejavu/ ישירות
const FONT = [
  "C:/Windows/Fonts/segoeui.ttf", "C:/Windows/Fonts/arial.ttf",
  "/usr/share/fonts/dejavu/DejaVuSans.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
...
async function renderMontage(aiaDir, project, opts = {}) {
  const id = project.id;
  const RENDER_DIR = path.join(aiaDir, "renders");
  const imgs = (project.assets || []).map((a) => path.join(aiaDir, "assets", id, a.name)).filter((f) => fs.existsSync(f));
  ...
  try {
    ...
    await finalize(RENDER_DIR, id, job, merged, { W, H, totalSec, opts, project });
    fs.rmSync(work, { recursive: true, force: true });
    return job;
  } catch (e) {
...
async function renderAnimation(aiaDir, project, opts = {}) {
  const id = project.id;
  const RENDER_DIR = path.join(aiaDir, "renders");
  const [W, H] = CANVAS[project.brief?.aspect] || CANVAS["16:9"];
...
async function finalize(RENDER_DIR, id, job, videoIn, { W, H, totalSec, opts, project }) {
...
async function render(aiaDir, project, opts = {}) {
  ...
  if (mode === "animation") return renderAnimation(aiaDir, project, opts);
  return renderMontage(aiaDir, project, opts);
}

function renderPath(aiaDir, id) {
  const p = path.join(aiaDir, "renders", `${id}.mp4`);
