  ...
  try {
    ...
    if (clips.length === 1) fs.copyFileSync(clips[0], merged);
      ...
      for (let i = 1; i < clips.length; i++) {
        ...
        fc += `[${last}][${i}:v]xfade=transition=fade:duration=${T}:offset=${offset.toFixed(3)}[${lbl}];`;
...
module.exports = { render, renderMontage, renderAnimation, jobState, renderPath, bedList, hasFFmpeg: !!ffmpegPath };
